...
**ブックマーク画面は「辞書の項目」（案2。`design/bookmarks.html`）。** 箱をやめて細罫で区切り、
上は2行だけ（切り替え＝件数入りの下線タブ／並び替え＋「例文」）。見出し・件数の札・説明文は外した。
- **「例文」の切替の中身は `state.density`**（`standard`＝例文あり／`compact`＝例文なし）。
  名前だけ変えたので、同期している値はそのまま
- **`compact` は例文を隠すだけ。字の大きさは変えない。**以前は語の字も縮めていたので、
  実機で「例文を押すと字が大きくなるだけ」と言われた
...
- **例文は見出し語からも引く。**古いブックマークは例文を持たず、idの形も今と違うことがあり、
  例文が1つも出ていなかった。例文を持つ項目が1つも無ければ「例文」ボタン自体を出さない
- 項目の器は `.bm-ent`。**`setLevel()` の `closest()` に `.bm-ent` が入っている**ので、
  器のクラス名を変えるならそこも直すこと（左の色罫がその場で変わらなくなる）
- 中身（語・拼音・意味・例文3行・読み上げ・難易度・★）は減らしていない。`tests/listen-test.mjs` が見張っている
...
```bash
node tests/run.mjs        # 全268項目＋Service Workerチェック（8スイート）
```
