...
- **単語タブ＝旧ブックマークの器（`#bookmarkPanel`、`renderBookmarkPanel()`）。**上に検索と絞り込み
  「すべて／★／苦手・曖昧／マイ単語」（`.w-chips`、`data-action="wf"`）。**選んだ絞り込みは `localStorage` の
...
- **並び替えは絞り込みごとに覚える。**すべて・苦手・曖昧は「Day順／拼音順／苦手順／ランダム」（`wSort()`／`wSortItems()`、
  `localStorage` の `hsk4-word-sort-all`／`-hard`。既定は Day順／苦手順）。★は従来の `state.bmSort`（おすすめ／苦手順／追加順／ランダム）。
...
行を押すとその下に、読み上げ・例文（中文・拼音・和訳）・難易度・「ブックマークを外す」・種類とDayが開く。
...
- 曖昧・苦手は行の左の色罫（`.bm-row::before`）。**`setLevel()` の `closest()` に `.bm-ent` が入っている**ので、
  器のクラス名を変えるならそこも直すこと（色罫がその場で変わらなくなる）
...
- `tests/listen-test.mjs` が「閉じた行に例文・難易度・読み上げを出さない」「開くと全部出る」などを見張っている
...
```bash
node tests/run.mjs        # 全314項目＋Service Workerチェック（8スイート）
```
