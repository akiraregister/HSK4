...
| `audio/w/*.mp3` | **単語・例文の録音**（`audio/w/README.md` 参照）。**まだ1本も作っていない**（`index.json` が空）。消音スイッチで読み上げが黙る件の確実な直し方。`audio/build-word-audio.mjs` で生成する |
...
- **⚠️ 原因は iOS の消音スイッチ。コードでは直せない（実機で確定）。**
  ...
  **確実に直すなら録音ファイルに替えるしかない**（下記「まだ手を付けていないこと」）
...
```bash
node tests/run.mjs        # 全247項目＋Service Workerチェック（8スイート）
```
...
- **単語・例文の読み上げが消音スイッチで黙る** — 上記「単語・例文の読み上げは『録音』では
  ...
  **アプリ側は実装済み**（録音を探して、無ければ読み上げへ落ちる）。
  **残っているのは音声を作ることだけ**で、手順は `audio/w/README.md` にある。
  規模は全部で**1,207本／7,693字／12〜16MB**、単語だけなら**498本／約2〜3MB**。
  Google Cloud TTSの無料枠（WaveNetで月100万字）に収まる。`GOOGLE_TTS_KEY` が要る。
  **起動時のダウンロードは増えない。**`sw.js` の `PRECACHE` は index.html と
  アイコンだけで、音声は**再生したものだけ**が端末に残る
  （既存の `audio/dayN.mp3` 13.3MB も同じ扱い）。1日ぶん学習して約120KB増える計算
