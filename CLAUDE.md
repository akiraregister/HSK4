...
| `lp/index.html` | 告知用ランディングページ。アプリと同じトークンを複製し、復習カードと並べ替えを実際に触れるデモとして載せている。**アプリを変えたらLPの記述も確かめること**（2026年9月、級診断の問数・画面見本の下タブ・「リスニングは入っていない」が古いままだった）。`tests/lp-test.mjs` が問数・下タブ・リスニングの記述をアプリと突き合わせている。価格の欄は**¥4,800（予定）**に揃えた（2026年9月）。関心の人数を数えるボタンは発売前の関心調べのまま残してある |
...
- 復習タブ（`#reviewTab` → `showTest()`）には今日ぶんの件数（`#revN`、`updRevBadge()`＝`revTodayCount()`。
  数え方は `renderTestScreen()` と同じ。**`renderTestScreen()` の頭でも更新する**：1問答えるたびと終了は
  ここだけを描き直すので、以前は別のタブへ移るまで件数が減らなかった）。タブの中なので「今日やることへ戻る」「あとにする」は外した
...
- **意味は必ず1行。**場所が足りない行は、まず語の側が縮んで拼音が中国語の下へ回り、それでも足りなければ
  意味の末尾を「…」で省く（長い文法だけ。幅440・「大」で18行）。**開いた行は意味を語の下へ1行まるごと回して全文を出す**。
  語の側は `min-width:min-content`（0まで縮めると長い文法名が意味の文字に重なった）
- **長い文法名は「：」の後ろで折り返す**（`wTitleHTML()` が `.zh-a`／`.zh-b` に分ける）。語と意味は同じ係数1で縮み、
  意味の下限は30%。**縮む係数を1未満にしないこと**：以前は意味が `.15` で、語の側が下限で止まったあと
  足りない幅の15%しか縮まず（flexの決まり）、長い文法名5つの行が画面の右へはみ出して意味と「›」が切れていた。
  `tests/listen-test.mjs` が440と375・「大」で全720行を見張っている
...
**ミニテストの答えた画面も1画面に収める**（実機で「スクロールで画面が揺れる」と言われた。81%の問題ではみ出していた）。
解説は「あなたの解答」と正解だけ出し、ほかの選択肢は畳む（`.mt-more`）。「テストをとばして完了する」は問番号の行の右
（`.mt-head`。`mtRender()` が `#mtSkip` をそこへ入れ、描き替える前に `mtParkSkip()` で外へ逃がす）。
**`.mt-exp .row` は `display:block`**（全体の `.row` がフレックスで、解説の文が「法律：　法律／は…」のように途中で折れていた）。
幅440×高さ956・文字サイズ「大」・ホーム画面起動の余白で、全Dayの問題を答えた状態ではみ出しゼロを確かめた
...
**ホーム画面への追加を勧める（2026年9月。Webで売るため）。**買った人の多くはブラウザで開いたまま使い、
しかも**iPhoneではSafariとホーム画面のアプリで保存場所が別**（Safariで学習・購入してから追加すると、
進み具合も購入も入っていない状態で始まる。iOSの仕様）。なのでスマホのブラウザで開いている人にだけ案内する。
- 判定は `appEnv()`（iOS／Android／アプリ内ブラウザ／`html.standalone`）。PCでは何も出さない
- 今日画面：`installRowHTML()` が復習の行の下に1行。ブラウザ＝「ホーム画面に追加しましょう」→ `showInstallGuide()`（手順のシート）。
  LINE・Instagram等のアプリ内ブラウザ＝「Safari（Chrome）で開き直して」（そこでは追加できない）。
  ホーム画面から初めて開いた・未学習・未ログイン＝「Safariで学習していましたか？ ログインで引き継げます」。
  ×で2週間隠す（`localStorage` の `hsk4-a2hs-hide-until`）
- 価格画面：`buyNoteHTML()` が購入ボタンの上に「購入はホーム画面のアプリから」。**購入は止めない（勧めるだけ）**
- Androidの Chrome が出す `beforeinstallprompt` は取っておき、シートの「いまインストールする」から呼ぶ
- **今日画面の「購入」と一覧の🔒のDayは、決済へ直行せず価格画面（`showDay()`）を通す。**以前は値段も上の案内も見ないまま決済へ飛んでいた
...
- **単語ステップにあった「消音スイッチを切って」の注意書き（`.spk-note`）は外した**（2026年9月）。
  全部録音に替えて消音でも鳴るようになり、事実と合わなくなったため。録音の無いマイ単語は下のトーストで気づける
...
```bash
node tests/run.mjs        # 全334項目＋Service Workerチェック（8スイート）
```
