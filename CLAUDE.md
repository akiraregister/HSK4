...
| 360-400行目付近 | **起動スプラッシュ**（`#splash`）。炎が灯り、芯がゆらぎ、ほのかに灯る。見えている時間は約1.8秒で、`splash-out` が1800msから470msかけて消し、`dismissSplash()` が2300msでDOMから外す。**この2つは必ず一緒に変えること** |
| 405-450行目付近 | **初回案内**（`.intro`）。3つの訴求を1つずつ順番に見せる。状態クラスは `intro-on`／`intro-done`（素の `.done` は完了画面のもので `text-align:center` が付くため、接頭辞が要る） |
...
- **スプラッシュの長さは2か所に書いてある。** CSSの `splash-out`（1800ms待って470msで消す）と
  JSの `dismissSplash()`（2300msでDOMから外す）。**片方だけ変えると、まだ見えているのに
  消えるか、消えたあとも居座るかのどちらかになる**
- **炎の灯り（`drop-shadow`）は案Xの「影ゼロ」から外した唯一の例外。**
  `--sh` は `none` のまま。灯りは `box-shadow` ではなく `filter:drop-shadow` を
  炎の`<g>`だけに掛けている（面に落ちる影にならないので罫の紙面を壊さない）。
  svg全体に掛けると足元の墨罫まで黄色く光る。**必ず0へ戻して、止まった影を残さない**
...
```bash
node tests/run.mjs        # 全199項目＋Service Workerチェック（7スイート）
```
