...
- **⚠️ 呼び方は実機で決まった形。理屈で変えないこと。**
  `ss.cancel()` を**毎回呼んでから**、`u.lang='zh-CN'` だけ指定して喋らせる。
  **`u.voice` に声のオブジェクトを渡さない。**
  2026年9月に「声を明示して選ぶ（zh-CN優先）」「鳴っている最中だけ cancel する」へ
  変えたところ、**それまで消音モードでも聞こえていたのが鳴らなくなった**。
  iOSの一般論としては良い変更のはずだったが、実機では逆効果だった。元の形に戻してある。
  `tests/listen-test.mjs` が「毎回 cancel する」「声を渡さない」の2点を見張っている
- **消音スイッチでも黙ることがある（別件）。**録音ファイル（MP3）は消音でも鳴るのに、
  読み上げは消音に従う場面がある。単語ステップの下に `.spk-note` で1行置いてある。
  消音かどうかは**webからは判定できない**ので出し分けはできず常時表示。
  確実に直すなら録音に替えるしかない（下記「まだ手を付けていないこと」）
- **`onstart` も `onend` も、音が出た証拠にはならない。**黙らされていてもiOSは普通に
  両方返してくる。**機械には「鳴ったか」は分からない**ので、`tools/speech-check.html`
  は最後に人へ「聞こえましたか」と尋ねる作りにしてある
  （最初は onstart で判定していて、**聞こえていないのに「鳴っています」と誤判定した**）
- **原因の切り分けで何度も外した。**消音スイッチ → PWA → 呼び方 → 消音 → 結局
  こちらの変更、という順に疑って回った。`tools/speech-check.html` に、録音MP3と
  読み上げを並べて試す仕掛けと、**書き方5通り**（A〜D に加えて **E＝変更前のアプリ
  そのもの**）を残してある。次に似たことが起きたら、**まずEを押して「以前の書き方なら
  鳴るか」を確かめること**。A〜Dだけ作ったときは、肝心のEの組み合わせが抜けていた
