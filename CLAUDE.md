...
| `audio/dayN.mp3` | リスニング問題の音声（Day 1〜80、Google Cloud TTS生成）。台本は `index.html` の `LISTENING` |
...
- **听力（リスニング）は Day 1〜80 まで実装済み・残り Day 81〜90 が未対応** — 台本は `LISTENING`（`index.html` 内）、音声は `audio/dayN.mp3`。Google Cloud TTS（Chirp3-HD Puck/Aoede）で生成。模試には未収録（`LISTENING` の問題はSRS復習・模試の対象外）
