...
```bash
node tests/run.mjs        # 全237項目＋Service Workerチェック（8スイート）
```
...
- **単語・例文の読み上げが消音スイッチで黙る** — 上記「単語・例文の読み上げは『録音』では
  ...
  本数だけ見られる）。規模は**1,207本／7,693字／12〜16MB**で、Google Cloud TTSの
  無料枠（WaveNetで月100万字）に収まる。実行には `GOOGLE_TTS_KEY` が要る。
  **起動時のダウンロードは増えない。**`sw.js` の `PRECACHE` は index.html と
  アイコンだけで、音声は**再生したものだけ**が stale-while-revalidate で端末に残る
  （既存の `audio/dayN.mp3` 13.3MB も同じ扱い）。1日ぶん学習して約120KB増える計算。
  小さく始めるなら**単語498本だけ（約2〜3MB）**でも効果がある（例文と文法例文が
  容量の8割）。アプリ側の切り替え（録音を探して、無ければ読み上げへ落ちる）は
  **まだ入れていない**ので、録音を作ったあとに書く必要がある
