...
- **iPhoneの消音スイッチで黙る（実機で確定）。**録音ファイル（MP3）は消音でも鳴るのに、
  **読み上げだけ消音スイッチに従う**。経路が違うため。2026年9月、実機で
  「リスニングは鳴るのに単語の読み上げだけ聞こえない」が起き、消音を解除したら鳴った。
  **利用者の端末でも同じことが起きる**（消音のまま使う人は多い）ので、単語ステップの
  下に `.spk-note` で1行置いてある。消音かどうかは**webからは判定できない**ので、
  出し分けはできず常時表示。確実に直すなら録音に替えるしかない（下記「まだ手を付けて
  いないこと」）
- **切り分けの寄り道で3回外した。**消音スイッチ → PWA → 呼び方、と疑って全部違った
  （最後に消音だった）。`tools/speech-check.html` に、録音MP3と読み上げを並べて試す
  仕掛けと、書き方4通り（cancelの有無／声指定の有無／速度）を残してある
...
```bash
node tests/run.mjs        # 全236項目＋Service Workerチェック（8スイート）
```
...
- **単語・例文の読み上げが消音スイッチで黙る** — 上記「単語・例文の読み上げは『録音』では
  ない」参照。いまは注意書きで逃げているが、**確実な直し方は録音ファイルに替えること**。
  `audio/build-word-audio.mjs` が生成器（**まだ一度も実行していない**。`--dry-run` で
  本数だけ見られる）。規模は**1,207本／7,693字／推定12MB**で、Google Cloud TTSの
  無料枠（WaveNetで月100万字）に収まる。実行には `GOOGLE_TTS_KEY` が要る。
  アプリ側の切り替え（録音を探して、無ければ読み上げへ落ちる）は**まだ入れていない**
  ので、録音を作ったあとに書く必要がある
