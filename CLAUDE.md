...
| `audio/w/*.mp3` | **単語の録音498本・3.4MB**（`audio/w/README.md` 参照）。消音スイッチで読み上げが黙る件の直し方。`audio/build-word-audio.mjs` で生成した（2026年9月）。**例文492本と文法例文217本はまだ**（`--words` を外して同じコマンドを叩けば作れる。容量は＋9〜13MB） |
...
- **例文と文法例文の録音がまだ** — 単語498本は作った（2026年9月）。残りは例文492本と
  文法例文217本で、**`--words` を外して同じコマンドを叩けば作れる**（手順は
  `audio/w/README.md`）。容量は＋9〜13MBの見込み。録音が無い文字列は端末の読み上げへ
  落ちるので、**作らなくても壊れない**（消音スイッチで黙るだけ）。
  **起動時のダウンロードは増えない。**`sw.js` の `PRECACHE` は index.html と
  アイコンだけで、音声は**再生したものだけ**が端末に残る
  （既存の `audio/dayN.mp3` 13.3MB も同じ扱い）
