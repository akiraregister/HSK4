...
**ブックマーク画面は「その場で開く」（案3-3。2026年9月25日）。** 一覧は**語・拼音・意味の1行だけ**。
...
- **マイ単語タブもブックマーク側と揃えてある**（見出し・件数の札・「学習画面に戻る」は無し。登録のしかたの説明は0語のときだけ）
...
```bash
node tests/run.mjs        # 全293項目＋Service Workerチェック（8スイート）
```
